...
public:
	...
	virtual float estimate(const board& b) const {
		...
		for(int i =0;i<8;i++){
			//計算address
            int address = indexof(isomorphic[i],b);
			//取出address中的值
			reward += weight[address];
		}
		return reward;
	}
...
public:
	 ...
	 */
	void update_episode(std::vector<state>& paths, float alpha = 0.1) const {
		// TODO
        float exact = 0;
        for (paths.pop_back() /* terminal state */; paths.size(); paths.pop_back()) {
            state& move = paths.back();
            float error = exact - estimate( move.after_state() );
            exact = move.reward()+ update( move.after_state() , 0.05 * error);
        }
